...
void main() {
  test('reports ordered phase and cumulative monotonic durations', () {
    ...
    var index = 0;
    final samples = <({String phase, Duration phaseTime, Duration totalTime})>[];
    ...
    expect(
      samples.map((sample) => sample.phaseTime.inMicroseconds),
      [40, 85, 0],
    );
    expect(
      samples.map((sample) => sample.totalTime.inMicroseconds),
      [40, 125, 125],
    );
  });
}
