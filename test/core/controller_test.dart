...
void main() {
  ...
  group('config methods', () {
    ...
    test(
      'generation controller APIs delegate without signature drift',
      () async {
        ...
        expect(
          await controller.validateCandidateConfigAtPath(
            '/managed/config.yaml',
          ),
          '',
        );
    ...
    test('isolated preparation admits its published candidate path', () async {
      ...
      when(
        () =>
            mock.activateRuleGeneration(profileId: 7, generation: 'generation'),
    ...
    test('restores activated generation when persistence fails', () async {
      ...
      when(
        () => mock.activateRuleGeneration(profileId: 7, generation: 'g'),
      ).thenAnswer((_) async => {'config-path': path});
      when(
        () => mock.restoreRuleGeneration(profileId: 7, failedGeneration: 'g'),
      ).thenAnswer((_) async => const {});
      final result = await controller.setupConfig(
        params: params,
        preparationConfig: 'rules: []',
    ...
    test(
      'restores activated generation when candidate validation fails',
      () async {
        const params = SetupParams(selectedMap: {}, testUrl: 'http://x.com');
        const path = '/app-group/prewarm/7/generations/g/config.yaml';
        when(
    ...
    test(
      'timed-out generation preparation cannot activate after completing late',
      () async {
        const params = SetupParams(selectedMap: {}, testUrl: 'http://x.com');
        final preparation = Completer<RuleGenerationPreparation>();
        ...
        final result = await controller.setupConfig(
          ...
          rulePreparationTimeout: const Duration(milliseconds: 10),
          prepareRuleGenerationOverride:
              ({required String config, required int profileId}) =>
                  preparation.future,
        );
