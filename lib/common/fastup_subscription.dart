...
Uint8List convertFastupSubscriptionBytes(Uint8List bytes) {
  ...
  for (final outbound in outbounds) {
    ...
    proxies.add({
      ...
      'password': password.endsWith('#fastup')
          ...
          : '$password#fastup',
      'mpw': mpw,
      if (tls['server_name'] case final String serverName
          when serverName.isNotEmpty)
        'sni': serverName,
  ...
  final clashConfig = <String, Object?>{
    'mixed-port': 7890,
    'allow-lan': false,
    'mode': 'rule',
    'log-level': 'info',
  ...
  return Uint8List.fromList(utf8.encode(yaml.encode(clashConfig)));
}
